MODULE MTRN4230_Server_Sample    
    ...
    PERS byte load := 0;    ! load = 0, unload = 1
    ...
    PROC ProcessConnection()
        ...
        ELSEIF requestMsg{1} = StrToByte("p" \Char) THEN   ! Client requested pose_state data
            ...
            read_joints := CJointT (\TaskRef:=T_ROB1Id);
        ...
        ELSEIF requestMsg{1} = StrToByte("L" \Char) THEN   ! Client wants to load/unload conveyor
